...
import (
	...
	messagingv1alpha1 "github.com/triggermesh/azure-event-channel/pkg/client/listers/messaging/v1alpha1"
	zap "go.uber.org/zap"
...
// controller reconciling v1alpha1.AzureChannel.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.AzureChannel. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.AzureChannel) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.AzureChannel.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.AzureChannel. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.AzureChannel) reconciler.Event
}

// reconcilerImpl implements controller.Reconciler for v1alpha1.AzureChannel resources.
type reconcilerImpl struct {
	// Client is used to write back status updates.
	Client internalclientset.Interface

	// Listers index properties about resources
	Lister messagingv1alpha1.AzureChannelLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client internalclientset.Interface, lister messagingv1alpha1.AzureChannelLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.AzureChannels(namespace)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.AzureChannel, desired *v1alpha1.AzureChannel) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.MessagingV1alpha1().AzureChannels(desired.Namespace)
		...
		updater := r.Client.MessagingV1alpha1().AzureChannels(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.AzureChannel) (*v1alpha1.AzureChannel, error) {

	getter := r.Lister.AzureChannels(resource.Namespace)
	...
	patcher := r.Client.MessagingV1alpha1().AzureChannels(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.AzureChannel) (*v1alpha1.AzureChannel, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.AzureChannel, reconcileEvent reconciler.Event) (*v1alpha1.AzureChannel, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
